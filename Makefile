...
ifndef MAJOR_VERSION
  MAJOR_VERSION = $(shell grep 'define MAJOR_VERSION' $(VERSION_H) | cut -d ' ' -f 3-)
endif
ifndef MINOR_VERSION
  MINOR_VERSION = $(shell grep 'define MINOR_VERSION' $(VERSION_H) | cut -d ' ' -f 3-)
endif
ifndef BUILD_TYPE
# might be: AlphaCode, BetaCode, ReleaseCode
  BUILD_TYPE = $(shell grep 'define BUILD_TYPE' $(VERSION_H) | cut -d ' ' -f 3-)
endif
ifndef BUILD_NUMBER
  BUILD_NUMBER = $(shell grep 'define BUILD_NUMBER' $(VERSION_H) | cut -d ' ' -f 3-)
endif
...
increment:
	-@BN=$(BUILD_NUMBER); \
        ...
        cp $(VERSION_H) $(TMP)/$(VERSION_H); \
        sed -e 's/BUILD_NUMBER.*'"$$BN"'/BUILD_NUMBER '"$$BNN/" \
                $(TMP)/$(VERSION_H) > $(VERSION_H); \
        rm -f $(TMP)/$(VERSION_H)
